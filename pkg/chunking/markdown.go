...
func NewOptimizedMarkdownChunker(config ChunkerConfig) (*OptimizedMarkdownChunker, error) {
	...
	chunker := &OptimizedMarkdownChunker{
		...
		mergeSparseParents: config.MergeSparseParents,
	}
	var err error
...
func NewMarkdownChunker(maxChunkSize, overlapSize int, preserveStructure bool) (*OptimizedMarkdownChunker, error) {
	config := ChunkerConfig{MaxChunkSize: maxChunkSize, MinChunkSize: maxChunkSize / 4, OverlapSize: overlapSize, PreserveStructure: preserveStructure, EnableSemantic: true, MergeSparseParents: true}
...
func (omc *OptimizedMarkdownChunker) ChunkMarkdown(content string) ([]Chunk, error) {
	...
	md := goldmark.New(goldmark.WithExtensions(extension.GFM, extension.Table), goldmark.WithParserOptions(parser.WithAutoHeadingID()))
	...
	optimized := omc.postProcessChunks(chunks)
...
func (omc *OptimizedMarkdownChunker) mergeSparseParentChunks(chunks []Chunk) []Chunk {
	...
	for i < len(chunks) {
		current := chunks[i]
		isSparseParent := current.Type == string(ChunkTypeSection) && current.TokenCount < sparseTokenThreshold && (i+1) < len(chunks)
		if isSparseParent {
			next := chunks[i+1]
			var builder strings.Builder
			builder.WriteString(strings.TrimSpace(current.Content))
			builder.WriteString("\n\n")
			builder.WriteString(next.Content)
			next.Content = builder.String()
			next.StartIndex = current.StartIndex
			next.TokenCount = omc.estimateTokenCount(next.Content)
			if next.Metadata == nil {
				next.Metadata = make(map[string]string)
			}
			next.Metadata["parent_section_title"] = current.Title
			optimizedChunks = append(optimizedChunks, next)
			i += 2
		} else {
			optimizedChunks = append(optimizedChunks, current)
			i++
		}
	}
...
func (omc *OptimizedMarkdownChunker) buildDocumentTree(doc ast.Node, source []byte) (*DocumentNode, error) {
	...
	root := &DocumentNode{Type: doc.Kind(), Level: 0, Title: "Document Root", Children: make([]*DocumentNode, 0)}
	...
	for len(stack) > 0 {
		...
		case *ast.Heading:
			...
			section := &DocumentNode{Type: frame.node.Kind(), Level: n.Level, Title: nodeInfo.Title, Content: nodeInfo.Content, StartIndex: nodeInfo.StartIndex, EndIndex: nodeInfo.EndIndex, Children: make([]*DocumentNode, 0), ASTNode: frame.node}
		...
		case *ast.Paragraph, *ast.CodeBlock, *ast.FencedCodeBlock, *ast.List:
			contentNode := &DocumentNode{Type: frame.node.Kind(), Level: 0, Content: nodeInfo.Content, StartIndex: nodeInfo.StartIndex, EndIndex: nodeInfo.EndIndex, ASTNode: frame.node}
...
func (omc *OptimizedMarkdownChunker) splitLargeContent(content string, section *DocumentNode, startID int) []Chunk {
	...
	for _, para := range paragraphs {
		...
		potential := currentChunk.String()
		if potential != "" {
			potential += "\n\n"
		}
		potential += para
		if len(potential) > omc.maxChunkSize && currentChunk.Len() > omc.minChunkSize {
			chunk := omc.createChunk(startID+chunkIndex, strings.TrimSpace(currentChunk.String()), section)
...
func (omc *OptimizedMarkdownChunker) createChunk(id int, content string, section *DocumentNode) Chunk {
	...
	chunk := Chunk{
		...
		Metadata:      map[string]string{"section_title": section.Title, "section_level": strconv.Itoa(section.Level), "chunk_type": string(chunkType)},
...
func (omc *OptimizedMarkdownChunker) isInlineNode(node ast.Node) bool {
	switch node.Kind() {
	case ast.KindText, ast.KindEmphasis, ast.KindLink, ast.KindImage, ast.KindCodeSpan, ast.KindAutoLink, ast.KindRawHTML, ast.KindString:
		return true
...
func (omc *OptimizedMarkdownChunker) estimateTokenCount(content string) int {
	...
	englishWords := 0
	for _, w := range words {
		if !omc.chineseRegex.MatchString(w) {
			englishWords++
		}
	}
	return chinese + int(float64(englishWords)*DefaultTokenRatio)
}
...
func (omc *OptimizedMarkdownChunker) getSimpleOverlap(content string) string {
	...
	start := len(content) - omc.overlapSize
...
func (omc *OptimizedMarkdownChunker) postProcessChunks(chunks []Chunk) []Chunk {
	for i := range chunks {
		chunks[i].Relationships = make([]string, 0, 3)
		if i > 0 {
